...
class Command(BaseCommand):
    ...
    def add_arguments(self, parser):
        parser.add_argument(
            '--scenario-id', 
            type=int, 
            help='Specific scenario ID to sync'
        )
        parser.add_argument(
            '--clear',
            action='store_true',
            help='Clear existing metrics before syncing'
        )
        parser.add_argument(
            '--batch-size',
            type=int,
    ...
    def handle(self, *args, **options):
        ...
        if scenario_id:
            ...
            self.stdout.write(f'Syncing metrics for all {scenarios.count()} scenarios...')
    ...
    def sync_scenario_metrics(self, scenario, clear_existing=False, batch_size=5000):
        ...
        if not numeric_fields and not string_fields:
            self.stdout.write(
                self.style.WARNING(f'  No summary fields configured for model {model.name}')
            )
            return
            ...
            for data in ScenarioData.objects.filter(scenario=scenario).iterator(chunk_size=1000):
                ...
                if len(metrics_to_create) >= batch_size:
                    ...
                    if processed % 10000 == 0:
                        ...
                        self.stdout.write(f'    {processed}/{total_records} ({progress:.1f}%)')
        ...
        self.stdout.write(
            self.style.SUCCESS(f'  ✓ Created {created} metrics for scenario {scenario.id}')
        )