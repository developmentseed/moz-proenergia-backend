...
class VectorDataset(models.Model):
    ...
    source = models.CharField(_("data owner"), max_length=155, blank=True, null=True)
...
class ReferenceDataset(models.Model):
    ...
    class Meta:
        ordering = ["id"]
        verbose_name = _("reference dataset")
        verbose_name_plural = _("reference datasets")


def generate_reference_file_name(instance, filename):
    """Generate a filename with the slugified dataset name
    and the file extension."""
    name, extension = splitext(filename)
    version = instance.dataset.files.count() + 1

    return f"reference/{slugify(instance.dataset.name)}_v{version}{extension}"
...
class ReferenceFile(models.Model):
    ...
    class Meta:
        ...
        verbose_name_plural = _("reference files")


@receiver(pre_delete, sender=ReferenceFile)
def delete_reference_file(sender, instance, **kwargs):
    """
    Delete the file from storage when a ReferenceFile instance is deleted
    """
    if instance.file:
        # Using default_storage for better compatibility with different storage backends
        if default_storage.exists(instance.file.name):
...
class DataModel(models.Model):
    ...
    reference_datasets = models.ManyToManyField(
        ReferenceDataset,
