...
from proenergia.datasets.models import DataModel, Scenario, ScenarioData, ScenarioDataMetrics, VectorDataset
...
class TestSyncScenarioMetricsCommand(TestCase):
    def setUp(self):
        ...
        self.scenario = Scenario.objects.create(
            ...
            vector_dataset=self.dataset
        )
        
        # Create test data with various scenarios
        self.test_data = [
            {"cost": 25000, "population": 1500, "location": "Maputo", "technology": "SHS"},
            {"cost": 123456.78, "population": 2300, "location": "Tete", "technology": "GridExtension"},
            {"cost": "45000.5", "population": "800", "location": "Beira", "technology": "MiniGrid"},
            {"cost": "invalid", "population": 1200, "location": None, "technology": "SHS"},
            {"cost": 75000, "population": None, "location": "Nampula", "technology": ""},
        ]
    ...
    def test_string_value_handling(self):
        ...
        # Test location field
        location_metrics = ScenarioDataMetrics.objects.filter(scenario=self.scenario, key='location')
        ...
        tech_metrics = ScenarioDataMetrics.objects.filter(scenario=self.scenario, key='technology')
    ...
    def test_scenario_id_option(self):
        ...
        self.assertTrue(ScenarioDataMetrics.objects.filter(scenario=self.scenario).exists())
        self.assertFalse(ScenarioDataMetrics.objects.filter(scenario=other_scenario).exists())
    ...
    def test_clear_option(self):
        ...
        initial_count = ScenarioDataMetrics.objects.filter(scenario=self.scenario).count()
        self.assertEqual(initial_count, 1)
        
        # Run command with clear
        out = StringIO()
        call_command('sync_scenario_metrics', scenario_id=self.scenario.id, clear=True, stdout=out)
    ...
    def test_no_summary_fields(self):
        ...
        empty_model = DataModel.objects.create(
            name="Empty Model",
            summary_fields=[]
        )
        ...
        self.assertFalse(ScenarioDataMetrics.objects.filter(scenario=empty_scenario).exists())
