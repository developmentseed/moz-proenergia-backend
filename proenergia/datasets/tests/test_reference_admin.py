...
from proenergia.datasets.models import ReferenceDataset, ReferenceFile


class ReferenceDatasetAdmin(TestCase):
    def setUp(self):
        self.client = Client()
    ...
    def test_creation_superuser(self):
        ...
        url = reverse("admin:datasets_referencedataset_add")
        ...
        dataset = ReferenceDataset.objects.get(name="Test Dataset")
    ...
    def test_admin_forbidden_update(self):
        ...
        self.client.login(username="admin_user", password="testpass123")
        url = reverse("admin:datasets_referencedataset_change", args=[dataset.id])
        response = self.client.get(url)
        self.assertEqual(response.status_code, 302)
    ...
    def test_superadmin_actions(self):
        ...
        self.client.login(username="superadmin", password="testpass123")
        url = reverse("admin:datasets_referencedataset_changelist")
        response = self.client.get(url)
        self.assertEqual(response.status_code, 200)
...
class ReferenceFileAdmin(TestCase):
    def setUp(self):
        self.client = Client()
        # Create superuser for admin access
        self.superadmin = get_user_model().objects.create_superuser(
    ...
    def test_reference_file_creation_superadmin(self):
        self.client.login(username="superadmin", password="testpass123")
        ...
        file = SimpleUploadedFile(
            "boundaries.pdf", b"file_content", content_type="application/pdf"
        )
        ...
        self.assertEqual(reference_file.created_by, self.superadmin)
        # Delete file
    ...
    def test_reference_file_creation_admin(self, mock_send_email):
        ...
        file = SimpleUploadedFile(
            "boundaries.csv", b"file_content", content_type="text/csv"
        )
